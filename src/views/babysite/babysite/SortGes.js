...
const SortGes = () => {
  ...
  const [docProgress, setDocProgress]     = useState({});  // { fieldName: 0-100 }
  const docInputRefs = useRef({});

  const uploadDocumentPdf = async (fieldName, file) => {
    if (!file) return;
    ...
    if (file.size > 10 * 1024 * 1024) {
      showNotification('danger', 'El PDF no debe superar 10MB');
      return;
    }

    try {
      setDocUploading(prev => ({ ...prev, [fieldName]: true }));
      setDocProgress(prev => ({ ...prev, [fieldName]: 0 }));

      // 1. Get signed upload params from backend
      const sigRes = await api.get('/api/upload/candidate-doc-signature', {
        params: { candidateId: id, docType: fieldName },
        withCredentials: true,
      });
      const { timestamp, signature, publicId, assetFolder, cloudName, apiKey } = sigRes.data;

      // 2. Build FormData — no transformation for PDFs
      const fd = new FormData();
      fd.append('file',          file);
      fd.append('api_key',       apiKey);
      fd.append('timestamp',     timestamp);
      ...
      // 3. Upload via XHR (with progress) to /raw/upload
      await new Promise((resolve, reject) => {
        const xhr = new XMLHttpRequest();
        xhr.open('POST', `https://api.cloudinary.com/v1_1/${cloudName}/raw/upload`);
        xhr.upload.onprogress = (e) => {
          if (e.lengthComputable)
            setDocProgress(prev => ({ ...prev, [fieldName]: Math.round(e.loaded * 100 / e.total) }));
        };
        xhr.onload = async () => {
          if (xhr.status !== 200) { reject(new Error('Upload failed')); return; }
          const { secure_url } = JSON.parse(xhr.responseText);

          // 4. Persist URL to DB immediately — send only this one field
          const DB_COLUMN_MAP = {
          ...
          const dbColumn = DB_COLUMN_MAP[fieldName] || `${fieldName}_url`;
          await api.put(`/api/sort-ges/${id}/checklist`, {
            [dbColumn]: secure_url,
          }, { withCredentials: true });

          // 5. Update local state — store the secure_url as the name so it persists on reload
          setDocumentos(prev => ({
            ...prev,
            [fieldName]: { name: secure_url, file: null, uploadedAt: new Date().toISOString() },
          }));
          setDocUploading(prev => ({ ...prev, [fieldName]: false }));
          setDocProgress(prev => ({ ...prev, [fieldName]: 0 }));
          showNotification('success', `PDF "${file.name}" subido correctamente`);
          resolve();
        };
        xhr.onerror = () => {
          setDocUploading(prev => ({ ...prev, [fieldName]: false }));
      ...
      setDocUploading(prev => ({ ...prev, [fieldName]: false }));
      showNotification('danger', 'Error al subir el PDF');
    }
  };
  ...
  const handleDocumentUpload = (fieldName, event) => {
    ...
    if (!file) return;
    uploadDocumentPdf(fieldName, file);
    // Reset input so same file can be re-selected if needed
    event.target.value = '';
  };

  const [docRemoving, setDocRemoving] = useState({});  // { fieldName: true/false }

  const handleRemoveDocument = async (fieldName) => {
    const doc = documentos[fieldName];
    // Only call the API if the doc was already uploaded to Cloudinary (has a URL)
    const isUploaded = doc?.name?.startsWith('http');

    try {
      setDocRemoving(prev => ({ ...prev, [fieldName]: true }));

      if (isUploaded) {
        await api.delete('/api/upload/candidate-doc', {
          params: { candidateId: id, docType: fieldName },
          withCredentials: true,
        });
      }
                ...
                <CAccordionItem itemKey={1}>
                  ...
                  <CAccordionBody>
                    <CRow>
                      <CCol md={6}>
                        {[
                          { key: 'certificado_nacimiento', label: 'Certificado de nacimiento:', inputId: 'doc_certificado_nacimiento' },
                          { key: 'curp',                   label: 'CURP:',                      inputId: 'doc_curp'                   },
                          { key: 'comprobante_domicilio',  label: 'Comprobante de domicilio:',   inputId: 'doc_comprobante_domicilio'  },
                          { key: 'poliza_seguro',          label: 'Póliza de seguro:',           inputId: 'doc_poliza_seguro'          },
                        ].map(({ key, label, inputId }) => {
                          const isUploading = docUploading[key];
                          const progress    = docProgress[key] || 0;
                          const doc         = documentos[key];
                          // If name starts with http it's already a Cloudinary URL
                          const isUrl       = doc?.name?.startsWith('http');
                          ...
                          return (
                            <div className="mb-3" key={key}>
                              ...
                              <div className="d-flex align-items-center gap-2">
                                {/* Hidden file input */}
                                <input
                                  type="file" accept="application/pdf" id={inputId}
                                  style={{ display: 'none' }}
                                ...
                                {isUploading ? (
                                  <div style={{ minWidth: '160px' }}>
                                    <CProgress value={progress} style={{ height: '6px' }} color="info" />
                                    <div className="text-muted" style={{ fontSize: '0.71rem', marginTop: 2 }}>
                                      Subiendo... {progress}%
                                    </div>
                                  </div>
                                ) : (
                                  <CButton
                                    ...
                                    style={!doc ? { borderColor: '#0071b8', color: '#0071b8' } : {}}
                                    variant="outline"
                                    onClick={() => document.getElementById(inputId).click()}
                                    className={`d-flex align-items-center gap-2 ${!doc ? 'pdf-upload-btn' : ''}`}
                                  >
                                    <CIcon icon={doc ? cilFile : cilCloudUpload} />
                                    {doc ? 'Reemplazar' : 'Subir PDF'}
                                  </CButton>
                                )}

                                {/* Cloudinary link / filename */}
                                {doc && !isUploading && (
                                  <>
                                    {isUrl ? (
                                      <CButton
                                        ...
                                        title="Ver PDF"
                                        onClick={() => {
                                          // Open the stream endpoint directly — backend pipes
                                          // the PDF through Express, bypassing Cloudinary's
                                          // untrusted account restriction entirely
                                          const params = new URLSearchParams({
                                            candidateId: id,
                                            docType: key,
                                          });
                                          window.open(
                                            `/api/upload/candidate-doc-stream?${params}`,
                                            '_blank',
                                            'noopener,noreferrer'
                                          );
                                        }}
                                      ...
                                      <span className="text-muted small text-truncate" style={{ maxWidth: '160px' }}>
                                        {doc.name}
                                      </span>
                                    )}
                                    <CButton
